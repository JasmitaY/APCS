...
/**
 *  This program prompts for a string of 3 to 15 letters, then searches the
...
public class EveryWord {
	...
	public void run() 
	{
		boolean done = false;
		do{
			String letters = Prompt.getString("\nEnter 3 to 15 letters");
			letters = letters.toLowerCase();
			// less than 3 or greater then 14 letters then quit
			if (letters.length() < 3 || letters.length() > 14)
			{
				done = true;
			}
			// not all letters; then print message
	...
	private boolean wordMatch (String word, String letters) 
	{
		for(int a = 0; a < word.length(); a++)
		{
			...
			// character doesn't appear in letters
			int index = letters.indexOf(c);
			if (index == -1)
			{
				return false;
			}	
			// take the character out of letters
